        ...
        <div className="lg:col-span-4 space-y-5">
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="relative px-6 pt-6 pb-4 bg-gradient-to-br from-white/5 to-transparent">
              <div className="flex flex-col items-center text-center">
                <div className="text-6xl md:text-7xl mb-3 drop-shadow-lg">{titleInfo.emoji}</div>
                <h2 className="text-2xl md:text-3xl font-bold mb-1">{player.name}</h2>
                <div className={`text-sm md:text-base font-bold ${titleInfo.color} mb-1.5`}>
                  {titleInfo.title}
                </div>
            ...
            <div className="px-4 pb-5 pt-2">
              <div className="text-xs md:text-xs uppercase tracking-widest font-bold text-muted-foreground/60 mb-3">Ünvan İlerlemesi</div>
                ...
                {PLAYER_TITLES.map((tier, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[9px] font-bold ${isCurrentTier ? tier.color : 'text-muted-foreground'} whitespace-nowrap`}>
                        {tier.minLevel}
                      </span>
                    </div>
                  );
              ...
              {nextTier && (
                <div className="mt-3 flex items-center gap-2 bg-black/30 border border-white/10 rounded-lg px-3 py-2.5">
                  <span className="text-xl grayscale">{nextTier.emoji}</span>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm md:text-sm font-bold text-muted-foreground truncate">{nextTier.title}</div>
                    <div className="text-xs text-muted-foreground/60">Seviye {nextTier.minLevel}</div>
                  </div>
                  {nextTier.milestoneCoins > 0 && (
                    <div className="text-xs font-mono text-yellow-400 font-bold shrink-0">+{nextTier.milestoneCoins} 🪙</div>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            <h3 className="font-bold text-base md:text-lg mb-4 flex items-center gap-2">
              <Award size={18} className="text-primary" /> Temel İstatistikler
            </h3>
            <div className="space-y-1">
              <StatRow icon={<Heart size={15} className="text-green-500" />}   label="Sağlık"     value={player.stats.maxHp}            base={player.baseStats.maxHp} />
              <StatRow icon={<Zap size={15} className="text-purple-500" />}    label="Mana"       value={player.stats.maxMana}          base={player.baseStats.maxMana} />
              <div className="border-t border-border/50 my-2" />
              <StatRow icon={<Sword size={15} className="text-orange-500" />}  label="Saldırı"   value={player.stats.attack}           base={player.baseStats.attack} />
              <StatRow icon={<Shield size={15} className="text-blue-500" />}   label="Savunma"   value={player.stats.defense}          base={player.baseStats.defense} />
              <StatRow icon={<Zap size={15} className="text-yellow-500" />}    label="Hız"        value={player.stats.speed}            base={player.baseStats.speed} />
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            ...
            <div className="space-y-1">
              <BattleStat icon={<Trophy size={14} className="text-yellow-400" />}  label="Zafer"            value={player.victories}                           color="text-yellow-400" />
              <BattleStat icon={<Skull size={14} className="text-destructive" />}  label="Ölüm"             value={player.deaths}                              color="text-destructive" />
              <BattleStat icon={<Sword size={14} className="text-red-400" />}      label="Toplam Öldürme"   value={player.totalKills ?? 0}                     color="text-red-400" />
              <BattleStat icon={<Wind size={14} className="text-cyan-400" />}      label="Başarılı Kaçış"   value={player.totalEscapes ?? 0}                   color="text-cyan-400" />
              <BattleStat icon={<Target size={14} className="text-purple-400" />}  label="Benzersiz Düşman" value={Object.keys(player.defeatedEnemies).length} color="text-purple-400" />
              <BattleStat icon={<Trophy size={14} className="text-orange-400" />}  label="Toplam Galip"     value={totalDefeated}                              color="text-orange-400" />
            </div>
            {player.strongestEnemy && (
              <div className="mt-4 pt-4 border-t border-border/50">
                <div className="text-xs uppercase tracking-widest font-bold text-muted-foreground/60 mb-2">En Güçlü Düşman</div>
                <div className="flex items-center gap-3 bg-black/30 border border-white/5 rounded-xl p-3">
                  <span className="text-3xl">{player.strongestEnemy.emoji}</span>
                  <div>
                    <div className="font-bold text-sm md:text-base">{player.strongestEnemy.name}</div>
                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                      <Star size={10} className="text-yellow-400" /> Seviye {player.strongestEnemy.level}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex gap-1 mb-5 bg-black/30 p-1 rounded-xl border border-border w-fit">
            {([['spells', '✨', 'Büyü Yükü'], ['equipment', '⚔️', 'Ekipman'], ['inventory', '🎒', 'Envanter']] as const).map(([key, icon, label]) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-xs md:text-sm transition-all ${
                  activeTab === key
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'spells' && (
              <motion.div key="spells" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                <h3 className="font-bold text-base md:text-lg mb-3 flex items-center gap-2">
                  ⚔️ Savaş Yükü
                  <span className="text-xs md:text-sm font-mono text-muted-foreground font-normal">({equippedSpells.length}/10)</span>
                </h3>
                <div className="grid grid-cols-5 gap-2 md:gap-3 mb-6">
                  {Array.from({ length: 10 }).map((_, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`absolute top-1 left-2 text-[9px] md:text-[10px] font-mono ${isSelected ? 'text-yellow-400' : 'text-muted-foreground/50'}`}>{idx + 1}</div>
                        ...
                        {spell ? (
                          <>
                            ...
                            <div className="text-[9px] md:text-[11px] font-semibold leading-tight truncate w-full text-center px-1">{spell.name}</div>
                            <div className="text-[8px] md:text-[10px] font-mono text-blue-400">{spell.manaCost}MP</div>
                          </>
                        ) : (
                          <>
                            <Plus size={16} className="text-white/20" />
                            <div className="text-[8px] md:text-[10px] text-muted-foreground/40">Boş</div>
                          </>
                        )}
                      </div>
                    );
                ...
                <h3 className="font-bold text-base md:text-lg mb-3 flex items-center gap-2">
                  📚 Öğrenilen Büyüler
                  <span className="text-xs md:text-sm font-mono text-muted-foreground font-normal">({unequippedKnown.length} eklenebilir)</span>
                </h3>
                {unequippedKnown.length === 0 ? (
                  <div className="text-center py-10 text-muted-foreground/50 text-sm md:text-base">
                    ...
                    {unequippedKnown.map(spellId => {
                      ...
                      return (
                        <button
                          ...
                          className={`bg-card border rounded-xl p-3 md:p-4 flex items-center gap-3 text-left transition-all ${
                        ...
                        >
                          <span className="text-2xl md:text-3xl shrink-0">{spell.emoji}</span>
                          <div className="min-w-0">
                            <div className="font-semibold text-xs md:text-sm leading-tight truncate">{spell.name}</div>
                            <div className="text-[9px] md:text-xs font-mono text-blue-400 mt-0.5">{spell.manaCost} MP</div>
                          </div>
                          <Plus size={14} className="text-primary/60 shrink-0 ml-auto" />
                        </button>
                      );
                    })}
                  </div>
            ...
            {activeTab === 'equipment' && (
              <motion.div key="equipment" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <h3 className="font-bold text-base md:text-lg mb-4">Kuşanılan Ekipman</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                  {slots.map(slot => {
                    const equippedId = player.equippedItems[slot.id];
                    const item = equippedId ? equipment[equippedId] : null;
                    return (
                      <div key={slot.id} className="bg-card border border-border rounded-xl p-4 flex flex-col items-center text-center relative group min-h-[140px] md:min-h-[160px]">
                        <div className="text-xs text-muted-foreground mb-2 uppercase tracking-widest flex items-center gap-1">
                          <span>{slot.emoji}</span> {slot.label}
                        </div>
                        {item ? (
                          <>
                            <div className="text-4xl md:text-5xl mb-2">{item.emoji}</div>
                            <div className="font-semibold text-xs md:text-sm leading-tight">{item.name}</div>
                            <div className="absolute inset-0 bg-black/85 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-xl">
                              <button
                                ...
                                className="px-3 py-2 bg-destructive/20 text-destructive border border-destructive/50 rounded-lg text-xs md:text-sm font-bold hover:bg-destructive/40 transition-colors"
                              >
                                Çıkar
                              </button>
                            </div>
                          ...
                          <div className="flex-1 flex flex-col items-center justify-center opacity-25">
                            ...
                            <span className="text-xs md:text-sm">Boş</span>
                          </div>
                        )}
                      </div>
                    );
                ...
                {Object.keys(player.equippedItems).length > 0 && (
                  <div className="mb-6 bg-black/20 border border-white/5 rounded-xl p-4">
                    <div className="text-xs uppercase tracking-widest font-bold text-muted-foreground/60 mb-3">Ekipman Bonusları</div>
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        ...
                        return Object.entries(bonuses).map(([k, v]) => (
                          <span key={k} className="text-xs md:text-sm px-2.5 py-1 bg-green-500/10 border border-green-500/20 rounded-lg text-green-400 font-mono font-semibold">+{v} {k}</span>
                        ));
                      })()}
                    </div>
                  </div>
            ...
            {activeTab === 'inventory' && (
              <motion.div key="inventory" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <h3 className="font-bold text-base md:text-lg mb-4">Envanter</h3>
                {player.inventory.length === 0 ? (
                  <div className="text-center py-20 text-muted-foreground/50">
                    <div className="text-4xl mb-3">🎒</div>
                    <p className="text-sm md:text-base">Envanterin boş. Marketten ekipman satın al.</p>
                  </div>
                ) : (
                    ...
                    {player.inventory.map(itemId => {
                      ...
                      return (
                        <div key={item.id} className={`bg-card border rounded-xl p-4 flex items-center gap-4 ${isEquipped ? 'border-primary/40' : 'border-border'}`}>
                          <div className={`text-3xl md:text-4xl w-12 h-12 md:w-14 md:h-14 rounded-lg flex items-center justify-center border shrink-0 ${isEquipped ? 'bg-primary/10 border-primary/30' : 'bg-black/40 border-white/10'}`}>
                            {item.emoji}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-muted-foreground capitalize mb-1.5">
                              {item.slot === 'silah' ? 'Silah' : item.slot === 'zirh' ? 'Zırh' : item.slot === 'yuzuk' ? 'Yüzük' : 'Kolye'}
                              {isEquipped && <span className="ml-2 text-primary font-semibold">· Kuşanıldı</span>}
                            </div>
                            <div className="flex flex-wrap gap-1 mt-1">
                              {Object.entries(item.statBonus).map(([k, v]) => (
                                <span key={k} className="text-[10px] md:text-xs px-1.5 py-0.5 bg-green-500/10 border border-green-500/20 rounded text-green-400 font-mono">+{v} {k.replace('max', '').toUpperCase()}</span>
                              ))}
                            </div>
                          </div>
                          {isEquipped ? (
                            <button
                              ...
                              className="shrink-0 px-3 py-2 bg-destructive/10 hover:bg-destructive/30 border border-destructive/30 text-destructive rounded-lg text-xs md:text-sm font-bold transition-colors"
                            >
                              Çıkar
                            </button>
                          ) : (
                            <button
                              ...
                              className="shrink-0 px-3 py-2 bg-primary/15 hover:bg-primary/30 border border-primary/30 text-white rounded-lg text-xs md:text-sm font-bold transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                            >
                              Kuşan
                            </button>
                          )}
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-border/20 last:border-0">
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-sm md:text-sm text-muted-foreground">{label}</span>
      </div>
      <span className={`font-mono font-bold text-base md:text-lg ${color}`}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex justify-between items-center py-2">
      ...
      <div className="flex items-center gap-2 font-mono">
        <span className="font-bold text-sm md:text-lg">{value}</span>
